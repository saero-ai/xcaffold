...
func (r *MemoryRenderer) Compile(config *ast.XcaffoldConfig, _ string) (*output.Output, []renderer.FidelityNote, error) {
	...
	for _, name := range names {
		notes = append(notes, renderer.FidelityNote{
			...
			Reason:     "GitHub Copilot has no native memory primitive; add this context to `.github/copilot-instructions.md` or a rule with activation: always.",
			Mitigation: fmt.Sprintf("Add the content of memory entry %q to .github/copilot-instructions.md, or declare a rule with activation: always in .xcaf.", name),
		})
	}
