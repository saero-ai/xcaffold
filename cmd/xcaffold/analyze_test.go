...
// no project.xcf yet — its primary purpose is to CREATE that file.
func TestAnalyzeCmd_DoesNotRequireXcfFile(t *testing.T) {
	// Change to a temp dir that has no project.xcf.
	tmp := t.TempDir()
	orig, err := os.Getwd()
	if err != nil {
	...
	if err := resolveProjectConfig(cmd); err != nil {
		t.Errorf("resolveProjectConfig returned error for 'analyze' in a dir with no project.xcf: %v", err)
	}
}
