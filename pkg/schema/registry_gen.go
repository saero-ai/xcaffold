// Code generated by gen-schema; DO NOT EDIT.

package schema

func init() {
	Registry["agent"] = KindSchema{
		Kind:    "agent",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this agent within the project.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "supported",
					"claude":       "unsupported",
					"codex":        "required",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Pattern: "^[a-z0-9-]+$",
				Role:    []string{"identity"},
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Human-readable purpose of this agent.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "optional",
					"claude":       "required",
					"codex":        "required",
					"copilot":      "required",
					"cursor":       "optional",
					"gemini":       "required",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Model",
				YAMLKey:     "model",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "LLM model identifier or alias resolved at compile time.",
				Group:       "Model & Execution",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "optional",
					"cursor":       "optional",
					"gemini":       "optional",
				},
				Example: "sonnet",
				Role:    []string{"rendering"},
			},
			{
				Name:        "Effort",
				YAMLKey:     "effort",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Reasoning effort level hint for the model provider.",
				Group:       "Model & Execution",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "MaxTurns",
				YAMLKey:     "max-turns",
				GoType:      "*int",
				XCAFType:    "integer",
				Optional:    true,
				Description: "Maximum conversation turns before the agent exits.",
				Group:       "Model & Execution",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "optional",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Tools",
				YAMLKey:     "tools",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Ordered list of tools this agent may invoke.",
				Group:       "Tool Access",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "optional",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "DisallowedTools",
				YAMLKey:     "disallowed-tools",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Tools explicitly denied to this agent.",
				Group:       "Tool Access",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Readonly",
				YAMLKey:     "readonly",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "When true, restricts the agent to read-only tool access.",
				Group:       "Tool Access",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "PermissionMode",
				YAMLKey:     "permission-mode",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Security mode controlling tool authorization behavior.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "DisableModelInvocation",
				YAMLKey:     "disable-model-invocation",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Prevents the agent from spawning sub-agents.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "UserInvocable",
				YAMLKey:     "user-invocable",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Whether users can invoke this agent directly via slash command.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Background",
				YAMLKey:     "background",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Runs the agent in background mode without interactive prompts.",
				Group:       "Lifecycle",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Isolation",
				YAMLKey:     "isolation",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Process isolation level for the agent session.",
				Group:       "Lifecycle",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Memory",
				YAMLKey:     "memory",
				GoType:      "FlexStringSlice",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Named memory banks attached to this agent.",
				Group:       "Memory & Context",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Color",
				YAMLKey:     "color",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Display color for terminal output differentiation.",
				Group:       "Memory & Context",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "InitialPrompt",
				YAMLKey:     "initial-prompt",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "System prompt prepended to every conversation.",
				Group:       "Memory & Context",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Skills",
				YAMLKey:     "skills",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Skill resource IDs attached to this agent.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"composition", "rendering"},
			},
			{
				Name:        "Rules",
				YAMLKey:     "rules",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Rule resource IDs governing this agent.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"composition"},
			},
			{
				Name:        "MCP",
				YAMLKey:     "mcp",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "MCP server resource IDs available to this agent.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"composition"},
			},
			{
				Name:        "Assertions",
				YAMLKey:     "assertions",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Policy assertion IDs evaluated post-compilation.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"composition"},
			},
			{
				Name:        "MCPServers",
				YAMLKey:     "mcp-servers",
				GoType:      "map[string]MCPConfig",
				XCAFType:    "map",
				Optional:    true,
				Description: "Inline MCP server definitions keyed by server name.",
				Group:       "Inline Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "optional",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Hooks",
				YAMLKey:     "hooks",
				GoType:      "HookConfig",
				XCAFType:    "HookConfig",
				Optional:    true,
				Description: "Inline lifecycle hook definitions for this agent.",
				Group:       "Inline Composition",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider override configuration keyed by provider name.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"filtering"},
			},
		},
	}
	Registry["blueprint"] = KindSchema{
		Kind:    "blueprint",
		Version: "1.0",
		Format:  "pure-yaml",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this blueprint.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this blueprint.",
				Group:       "Identity",
			},
			{
				Name:        "Extends",
				YAMLKey:     "extends",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Name of another blueprint to inherit selections from.",
				Group:       "Inheritance",
			},
			{
				Name:        "Agents",
				YAMLKey:     "agents",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Agent resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Skills",
				YAMLKey:     "skills",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Skill resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Rules",
				YAMLKey:     "rules",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Rule resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Workflows",
				YAMLKey:     "workflows",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Workflow resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "MCP",
				YAMLKey:     "mcp",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "MCP server resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Policies",
				YAMLKey:     "policies",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Policy resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Memory",
				YAMLKey:     "memory",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Memory resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Contexts",
				YAMLKey:     "contexts",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Context resource IDs to include in this blueprint.",
				Group:       "Resource Selectors",
			},
			{
				Name:        "Settings",
				YAMLKey:     "settings",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Name of the settings block to use.",
				Group:       "Singleton Selectors",
			},
			{
				Name:        "Hooks",
				YAMLKey:     "hooks",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Name of the hooks block to use.",
				Group:       "Singleton Selectors",
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Compilation targets for this blueprint. When set, controls which providers this blueprint compiles for. When empty, falls through to project.targets or --target flag.",
				Group:       "Multi-Target",
				Role:        []string{"filtering"},
			},
		},
	}
	Registry["context"] = KindSchema{
		Kind:    "context",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this context block.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this context block.",
				Group:       "Identity",
				Provider: map[string]string{
					"cursor": "optional",
				},
			},
			{
				Name:        "Default",
				YAMLKey:     "default",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Marks this context as tie-breaker when multiple match the same target.",
				Group:       "Behavior",
				Provider: map[string]string{
					"cursor": "optional",
				},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Restricts this context to specific provider targets.",
				Group:       "Targeting",
			},
			{
				Name:        "Path",
				YAMLKey:     "path",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Relative subdirectory for the rendered instruction file.",
				Group:       "Targeting",
			},
		},
	}
	Registry["hooks"] = KindSchema{
		Kind:    "hooks",
		Version: "1.0",
		Format:  "pure-yaml",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this hook block.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "supported",
				},
				Pattern: "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this hook block.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
			},
			{
				Name:        "Artifacts",
				YAMLKey:     "artifacts",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Named subdirectories to copy from xcaf/hooks/<name>/ to provider hook dirs.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
			},
			{
				Name:        "Events",
				YAMLKey:     "events",
				GoType:      "HookConfig",
				XCAFType:    "HookConfig",
				Optional:    true,
				Description: "Lifecycle event handlers keyed by event name.",
				Group:       "Events",
				Provider: map[string]string{
					"antigravity2": "supported",
					"claude":       "optional",
				},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider behavioral overrides for this hook block.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
			},
		},
	}
	Registry["mcp"] = KindSchema{
		Kind:    "mcp",
		Version: "1.0",
		Format:  "pure-yaml",
		Fields: []Field{
			{
				Name:        "Env",
				YAMLKey:     "env",
				GoType:      "map[string]string",
				XCAFType:    "map",
				Optional:    true,
				Description: "Environment variables passed to the server process.",
				Group:       "Environment & Headers",
				Provider: map[string]string{
					"antigravity2": "optional",
					"claude":       "optional",
					"copilot":      "optional",
					"gemini":       "optional",
				},
			},
			{
				Name:        "Headers",
				YAMLKey:     "headers",
				GoType:      "map[string]string",
				XCAFType:    "map",
				Optional:    true,
				Description: "HTTP headers for remote server connections.",
				Group:       "Environment & Headers",
				Provider: map[string]string{
					"antigravity2": "optional",
					"claude":       "optional",
				},
			},
			{
				Name:        "Disabled",
				YAMLKey:     "disabled",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "When true, the server is registered but not started.",
				Group:       "Control",
				Provider: map[string]string{
					"antigravity2": "unsupported",
					"claude":       "optional",
				},
			},
			{
				Name:        "OAuth",
				YAMLKey:     "oauth",
				GoType:      "map[string]string",
				XCAFType:    "map",
				Optional:    true,
				Description: "OAuth configuration key-value pairs for authentication.",
				Group:       "Authentication",
				Provider: map[string]string{
					"antigravity2": "unsupported",
					"claude":       "optional",
				},
			},
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this MCP server.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
				Pattern: "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this MCP server.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
			},
			{
				Name:        "Type",
				YAMLKey:     "type",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Server type (stdio or sse).",
				Group:       "Connection",
				Provider: map[string]string{
					"antigravity2": "optional",
					"claude":       "optional",
					"copilot":      "optional",
					"gemini":       "optional",
				},
			},
			{
				Name:        "Command",
				YAMLKey:     "command",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Shell command to start the MCP server process.",
				Group:       "Connection",
				Provider: map[string]string{
					"antigravity2": "optional",
					"claude":       "optional",
					"copilot":      "optional",
					"gemini":       "optional",
				},
			},
			{
				Name:        "URL",
				YAMLKey:     "url",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "URL for remote MCP servers.",
				Group:       "Connection",
				Provider: map[string]string{
					"antigravity2": "supported",
					"claude":       "optional",
					"copilot":      "optional",
					"gemini":       "optional",
				},
			},
			{
				Name:        "Cwd",
				YAMLKey:     "cwd",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Working directory for the MCP server process.",
				Group:       "Connection",
				Provider: map[string]string{
					"antigravity2": "unsupported",
					"claude":       "optional",
				},
			},
			{
				Name:        "AuthProviderType",
				YAMLKey:     "auth-provider-type",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Authentication provider type for remote servers.",
				Group:       "Authentication",
				Provider: map[string]string{
					"antigravity2": "unsupported",
					"claude":       "optional",
				},
			},
			{
				Name:        "Args",
				YAMLKey:     "args",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Arguments passed to the server command.",
				Group:       "Connection",
				Provider: map[string]string{
					"antigravity2": "optional",
					"claude":       "optional",
					"copilot":      "optional",
					"gemini":       "optional",
				},
			},
			{
				Name:        "DisabledTools",
				YAMLKey:     "disabled-tools",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Tool names to disable on this MCP server.",
				Group:       "Control",
				Provider: map[string]string{
					"antigravity2": "supported",
					"claude":       "optional",
				},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider behavioral overrides for this MCP server.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity2": "optional",
				},
			},
		},
	}
	Registry["memory"] = KindSchema{
		Kind:    "memory",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this memory entry.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this memory entry.",
				Group:       "Identity",
			},
		},
	}
	Registry["policy"] = KindSchema{
		Kind:    "policy",
		Version: "1.0",
		Format:  "pure-yaml",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this policy.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this policy.",
				Group:       "Identity",
			},
			{
				Name:        "Severity",
				YAMLKey:     "severity",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Violation severity level when the policy fails.",
				Group:       "Evaluation",
				Enum:        []string{"error", "warning", "off"},
			},
			{
				Name:        "Target",
				YAMLKey:     "target",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Resource kind this policy evaluates.",
				Group:       "Evaluation",
				Enum:        []string{"agent", "skill", "rule", "hook", "settings", "output"},
			},
			{
				Name:        "Match",
				YAMLKey:     "match",
				GoType:      "*PolicyMatch",
				XCAFType:    "*PolicyMatch",
				Optional:    true,
				Description: "Filter conditions selecting which resources to evaluate.",
				Group:       "Match Filter",
			},
			{
				Name:        "Require",
				YAMLKey:     "require",
				GoType:      "[]PolicyRequire",
				XCAFType:    "[]PolicyRequire",
				Optional:    true,
				Description: "Field constraints applied to matched resources.",
				Group:       "Requirements",
			},
			{
				Name:        "Deny",
				YAMLKey:     "deny",
				GoType:      "[]PolicyDeny",
				XCAFType:    "[]PolicyDeny",
				Optional:    true,
				Description: "Forbidden content or path patterns in compiled output.",
				Group:       "Deny Rules",
			},
		},
	}
	Registry["rule"] = KindSchema{
		Kind:    "rule",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "AlwaysApply",
				YAMLKey:     "always-apply",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "When true, applies this rule to all files unconditionally.",
				Group:       "Activation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this rule.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Activation",
				YAMLKey:     "activation",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Cross-provider activation mode for this rule.",
				Group:       "Activation",
				Enum:        []string{"always", "path-glob", "model-decided", "manual-mention", "explicit-invoke"},
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this rule within the project.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Pattern: "^[a-z0-9-]+$",
				Role:    []string{"identity"},
			},
			{
				Name:        "Paths",
				YAMLKey:     "paths",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Glob patterns for path-based activation.",
				Group:       "Activation",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "ExcludeAgents",
				YAMLKey:     "exclude-agents",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Agent types excluded from receiving this rule.",
				Group:       "Provider-Specific",
				Enum:        []string{"code-review", "cloud-agent"},
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider override configuration keyed by provider name.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"filtering"},
			},
		},
	}
	Registry["settings"] = KindSchema{
		Kind:    "settings",
		Version: "1.0",
		Format:  "pure-yaml",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this settings block.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this settings block.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Agent",
				YAMLKey:     "agent",
				GoType:      "any",
				XCAFType:    "any",
				Optional:    true,
				Description: "Agent configuration object passed through to the provider.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Worktree",
				YAMLKey:     "worktree",
				GoType:      "any",
				XCAFType:    "any",
				Optional:    true,
				Description: "Worktree configuration object passed through to the provider.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "AutoMode",
				YAMLKey:     "auto-mode",
				GoType:      "any",
				XCAFType:    "any",
				Optional:    true,
				Description: "Auto-mode behavior configuration.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "CleanupPeriodDays",
				YAMLKey:     "cleanup-period-days",
				GoType:      "*int",
				XCAFType:    "integer",
				Optional:    true,
				Description: "Days before unused sessions are cleaned up.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "IncludeGitInstructions",
				YAMLKey:     "include-git-instructions",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Whether to include git instructions in agent context.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "SkipDangerousModePermissionPrompt",
				YAMLKey:     "skip-dangerous-mode-permission-prompt",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Skip the dangerous mode permission confirmation prompt.",
				Group:       "Permissions",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Permissions",
				YAMLKey:     "permissions",
				GoType:      "*PermissionsConfig",
				XCAFType:    "*PermissionsConfig",
				Optional:    true,
				Description: "Tool permission rules (allow, deny, ask lists).",
				Group:       "Permissions",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Sandbox",
				YAMLKey:     "sandbox",
				GoType:      "*SandboxConfig",
				XCAFType:    "*SandboxConfig",
				Optional:    true,
				Description: "OS-level process isolation configuration.",
				Group:       "Permissions",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "AutoMemoryEnabled",
				YAMLKey:     "auto-memory-enabled",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Enable automatic memory persistence.",
				Group:       "Memory",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "DisableAllHooks",
				YAMLKey:     "disable-all-hooks",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Disable all lifecycle hooks globally.",
				Group:       "Hooks",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Attribution",
				YAMLKey:     "attribution",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Enable commit attribution metadata.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "MCPServers",
				YAMLKey:     "mcp-servers",
				GoType:      "map[string]MCPConfig",
				XCAFType:    "map",
				Optional:    true,
				Description: "Inline MCP server definitions.",
				Group:       "MCP",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Hooks",
				YAMLKey:     "hooks",
				GoType:      "HookConfig",
				XCAFType:    "HookConfig",
				Optional:    true,
				Description: "Lifecycle hook definitions.",
				Group:       "Hooks",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "StatusLine",
				YAMLKey:     "status-line",
				GoType:      "*StatusLineConfig",
				XCAFType:    "*StatusLineConfig",
				Optional:    true,
				Description: "Status line display configuration.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "RespectGitignore",
				YAMLKey:     "respect-gitignore",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Whether to respect .gitignore when scanning files.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Env",
				YAMLKey:     "env",
				GoType:      "map[string]string",
				XCAFType:    "map",
				Optional:    true,
				Description: "Environment variables injected into agent sessions.",
				Group:       "Environment",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "EnabledPlugins",
				YAMLKey:     "enabled-plugins",
				GoType:      "map[string]bool",
				XCAFType:    "map",
				Optional:    true,
				Description: "Plugin enable/disable flags keyed by plugin name.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "DisableSkillShellExecution",
				YAMLKey:     "disable-skill-shell-execution",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Prevent skills from executing shell commands.",
				Group:       "Permissions",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "AlwaysThinkingEnabled",
				YAMLKey:     "always-thinking-enabled",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Force extended thinking on every turn.",
				Group:       "Model",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "EffortLevel",
				YAMLKey:     "effort-level",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Default reasoning effort level.",
				Group:       "Model",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "DefaultShell",
				YAMLKey:     "default-shell",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Default shell for command execution.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Language",
				YAMLKey:     "language",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "UI language preference.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "OutputStyle",
				YAMLKey:     "output-style",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Output verbosity style.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "PlansDirectory",
				YAMLKey:     "plans-directory",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Directory for storing plan files.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Model",
				YAMLKey:     "model",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Default model for the platform session.",
				Group:       "Model",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "OtelHeadersHelper",
				YAMLKey:     "otel-headers-helper",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Helper command for generating OpenTelemetry headers.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "AutoMemoryDirectory",
				YAMLKey:     "auto-memory-directory",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Directory for automatic memory file storage.",
				Group:       "Memory",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "AvailableModels",
				YAMLKey:     "available-models",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "List of model IDs available for selection.",
				Group:       "Model",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "MdExcludes",
				YAMLKey:     "md-excludes",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Glob patterns for paths excluded from root context file scanning.",
				Group:       "Platform",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider behavioral overrides for this settings block.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity2": "unsupported",
				},
			},
		},
	}
	Registry["skill"] = KindSchema{
		Kind:    "skill",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this skill within the project.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "required",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Pattern: "^[a-z0-9-]+$",
				Role:    []string{"identity"},
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this skill.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "required",
					"claude":       "optional",
					"codex":        "required",
					"copilot":      "optional",
					"cursor":       "optional",
					"gemini":       "optional",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "WhenToUse",
				YAMLKey:     "when-to-use",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Guidance for the model on when to invoke this skill.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "License",
				YAMLKey:     "license",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "SPDX license identifier for open-source skills.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "AllowedTools",
				YAMLKey:     "allowed-tools",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Tools this skill is permitted to use. Skill-specific field.",
				Group:       "Tool Access",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "optional",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "DisableModelInvocation",
				YAMLKey:     "disable-model-invocation",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Prevents the skill from spawning sub-agents.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "UserInvocable",
				YAMLKey:     "user-invocable",
				GoType:      "*bool",
				XCAFType:    "boolean",
				Optional:    true,
				Description: "Whether users can invoke this skill directly via slash command.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "ArgumentHint",
				YAMLKey:     "argument-hint",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Hint text shown to the user when invoking the skill.",
				Group:       "Permissions & Invocation",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "Paths",
				YAMLKey:     "paths",
				GoType:      "ClearableList",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Glob patterns that scope this skill to matching files.",
				Group:       "Scoping",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "optional",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"rendering"},
			},
			{
				Name:        "Metadata",
				YAMLKey:     "metadata",
				GoType:      "map[string]string",
				XCAFType:    "map[string]string",
				Optional:    true,
				Description: "Arbitrary key-value mapping for additional metadata.",
				Group:       "Metadata",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "unsupported",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "optional",
					"gemini":       "unsupported",
				},
				Role: []string{"metadata"},
			},
			{
				Name:        "Artifacts",
				YAMLKey:     "artifacts",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Named subdirectories to copy from xcaf/skills/<id>/ to provider output.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity":  "optional",
					"antigravity2": "optional",
					"claude":       "optional",
					"codex":        "optional",
					"copilot":      "optional",
					"cursor":       "optional",
					"gemini":       "optional",
				},
				Role: []string{"composition"},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider override configuration keyed by provider name.",
				Group:       "Multi-Target",
				Provider: map[string]string{
					"antigravity":  "unsupported",
					"antigravity2": "optional",
					"claude":       "unsupported",
					"codex":        "unsupported",
					"copilot":      "unsupported",
					"cursor":       "unsupported",
					"gemini":       "unsupported",
				},
				Role: []string{"filtering"},
			},
		},
	}
	Registry["template"] = KindSchema{
		Kind:    "template",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this template within the project.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this template.",
				Group:       "Identity",
			},
			{
				Name:        "DefaultTarget",
				YAMLKey:     "default-target",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Default compilation target provider for this template.",
				Group:       "Compilation",
			},
		},
	}
	Registry["workflow"] = KindSchema{
		Kind:    "workflow",
		Version: "1.0",
		Format:  "frontmatter+body",
		Fields: []Field{
			{
				Name:        "Name",
				YAMLKey:     "name",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    false,
				Description: "Unique identifier for this workflow within the project.",
				Group:       "Identity",
				Pattern:     "^[a-z0-9-]+$",
			},
			{
				Name:        "Description",
				YAMLKey:     "description",
				GoType:      "string",
				XCAFType:    "string",
				Optional:    true,
				Description: "Human-readable purpose of this workflow.",
				Group:       "Identity",
				Provider: map[string]string{
					"antigravity": "optional",
				},
			},
			{
				Name:        "Steps",
				YAMLKey:     "steps",
				GoType:      "[]WorkflowStep",
				XCAFType:    "[]WorkflowStep",
				Optional:    true,
				Description: "Ordered procedural steps for multi-step workflows.",
				Group:       "Steps",
				Provider: map[string]string{
					"antigravity": "optional",
				},
			},
			{
				Name:        "Targets",
				YAMLKey:     "targets",
				GoType:      "map[string]TargetOverride",
				XCAFType:    "map",
				Optional:    true,
				Description: "Per-provider overrides and lowering-strategy directives.",
				Group:       "Multi-Target",
			},
			{
				Name:        "Activation",
				YAMLKey:     "activation",
				GoType:      "*Activation",
				XCAFType:    "Activation",
				Optional:    true,
				Description: "Activation mode: \"always\" (all contexts) or a list of path globs for conditional triggering.",
				Group:       "Activation",
				Provider: map[string]string{
					"antigravity": "optional",
					"claude":      "optional",
					"copilot":     "optional",
					"cursor":      "optional",
					"gemini":      "optional",
				},
			},
			{
				Name:        "Artifacts",
				YAMLKey:     "artifacts",
				GoType:      "[]string",
				XCAFType:    "[]string",
				Optional:    true,
				Description: "Named subdirectories to copy from xcaf/workflows/<id>/ to provider output.",
				Group:       "Composition",
				Provider: map[string]string{
					"antigravity": "optional",
					"claude":      "optional",
					"copilot":     "optional",
					"cursor":      "optional",
					"gemini":      "optional",
				},
			},
		},
	}
}
