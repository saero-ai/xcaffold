...
// MigrateStateFiles detects scaffold*.lock files in baseDir, converts them
// into a StateManifest, and writes to .xcaffold/project.xcf.state.
// Original lock files are renamed to *.lock.migrated.
// Returns true if migration was performed, false if no lock files found.
func MigrateStateFiles(baseDir string) (bool, error) {
	pattern := filepath.Join(filepath.Clean(baseDir), "scaffold*.lock")
	matches, err := filepath.Glob(pattern)
	if err != nil {
		return false, fmt.Errorf("failed to glob lock files: %w", err)
	}
	if len(matches) == 0 {
		return false, nil
	}
	...
	stateDir := StateDir(baseDir)
	statePath := StateFilePath(baseDir, "")
	if _, err := os.Stat(statePath); err == nil {
		return false, nil // already migrated
	}

	// Read each lock file and merge into a single StateManifest
	manifest := &StateManifest{
		Version:         stateFileVersion,
		XcaffoldVersion: XcaffoldVersion,
		Targets:         make(map[string]TargetState),
	...
	for _, lockPath := range matches {
		...
		// Parse as generic YAML to extract target and artifacts
		var legacy struct {
			Target      string       `yaml:"target"`
			...
			MemorySeeds []MemorySeed `yaml:"memory_seeds"`
		}
		...
		// Use source files from the first lock file that has them
		if len(manifest.SourceFiles) == 0 && len(legacy.SourceFiles) > 0 {
			manifest.SourceFiles = legacy.SourceFiles
		}

		// Merge memory seeds
		if len(legacy.MemorySeeds) > 0 {
			manifest.MemorySeeds = append(manifest.MemorySeeds, legacy.MemorySeeds...)
		}
	}
	...
	if err := WriteState(manifest, statePath); err != nil {
		return false, fmt.Errorf("failed to write migrated state: %w", err)
	}

	// Rename original lock files
	for _, lockPath := range matches {
		os.Rename(lockPath, lockPath+".migrated") //nolint:errcheck
	}
