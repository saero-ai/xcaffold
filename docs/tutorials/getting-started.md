...
Open `project.xcf` in your editor. It acts as the project manifest, linking your targeted platforms with your resource documents:
...
The lock file records SHA-256 hashes of both the source (`project.xcf`) and the compiled output files. Each target has its own lock file (`scaffold.gemini.lock`, `scaffold.cursor.lock`, etc.) so multiple targets can be maintained simultaneously without invalidating each other.
...
Lock files are committed to version control alongside `project.xcf`. Together they give you a reproducible, auditable agent configuration.
...
You created a `project.xcf` file with a project manifest and one agent definition, compiled it to `.claude/agents/developer.md` using the `claude` target, and observed how the lock file records SHA-256 hashes of both source and output. You verified that xcaffold recompiles only when the source changes, and that the lock prevents unrecorded drift on subsequent applies.
