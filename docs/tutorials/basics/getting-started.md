...
xcaffold detects the first available agent runtime on your `$PATH` and uses it as the suggested default target. All targets — `antigravity`, `claude`, `copilot`, `cursor`, and `gemini` — are fully supported and selected via `--target` on apply.
  ...
  xcaf/
    agents/
      xaff/
        ...
        agent.antigravity.xcaf  # provider override (antigravity target)
    rules/
      xcaf-conventions/
        rule.xcaf            # kind: rule
    skills/
