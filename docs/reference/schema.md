...
Defines a project-level constraint evaluated during compilation. Policies are authored in separate `.xcf` files with `kind: policy`. They are executed deterministically before compilation to enforce architecture scopes.

| Field | Type | Required | Description |
|---|---|---|---|
| `kind` | `string` | **Required** | Must be exactly `"policy"`. Distinguishes the file from the compiler payload. |
| `name` | `string` | **Required** | The unique identifier for the policy (e.g., `"path-safety"`). Used to match overrides and identify the policy in diagnostics. |
| `description` | `string` | Optional | Human-readable explanation of why the policy exists. |
| `severity` | `string` | **Required** | Action to take on violation. Values: `"error"` (fails compiler, blocks write), `"warning"` (prints diagnostic, allows write), `"off"` (suppresses policy). |
| `target` | `string` | **Required** | The resource type this policy evaluates against (e.g., `"agent"`, `"skill"`, `"output"`). |
| `match` | `PolicyMatch` | Optional | Conditions that select which resources the policy applies to. See [PolicyMatch](#policymatch). |
| `require` | `[]PolicyRequire` | Optional | Field-level requirements enforced on matched resources. See [PolicyRequire](#policyrequire). |
| `deny` | `[]PolicyDeny` | Optional | Content patterns that cause a violation when detected. See [PolicyDeny](#policydeny). |

### `PolicyMatch`

Defines selection criteria within `policy.match`. Determines which resources the policy applies to.

| Field | Type | Required | Description |
|---|---|---|---|
| `has_tool` | `string` | Optional | Matches resources that include the specified tool (e.g., `"Bash"`). |
| `has_field` | `string` | Optional | Matches resources that define the specified field. |
| `name_matches` | `string` | Optional | Regex pattern matched against the resource name. |
| `target_includes` | `string` | Optional | Matches resources whose target configuration includes the specified value. |

### `PolicyRequire`

Defines field-level requirements within `policy.require`. Each entry enforces a constraint on a specific field of the matched resource.

| Field | Type | Required | Description |
|---|---|---|---|
| `field` | `string` | **Required** | The field name to validate (e.g., `"description"`, `"tools"`). |
| `is_present` | `*bool` | Optional | When `true`, the field must exist and be non-empty. |
| `min_length` | `*int` | Optional | Minimum string length or minimum element count for the field value. |
| `max_count` | `*int` | Optional | Maximum element count for list-type fields. |
| `one_of` | `[]string` | Optional | The field value must be one of the specified strings. |

### `PolicyDeny`

Defines content denial patterns within `policy.deny`. Each entry triggers a violation when the pattern is detected.

| Field | Type | Required | Description |
|---|---|---|---|
| `content_matches` | `string` | Optional | A regular expression pattern matched against compiled output content. |
| `path_contains` | `string` | Optional | Substring matched against output file paths. |
| `content_contains` | `[]string` | Optional | Literal strings that must not appear in compiled output content. |

> [!NOTE]
> Policies evaluate against the in-memory compiler graph (`*ast.XcaffoldConfig`) natively. Custom assertions using regex, field requirements, or content denial patterns are handled by Go evaluator strategies inside `xcaffold/internal/policy/evaluator.go`.
