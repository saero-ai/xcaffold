...
For providers that do not compile `kind: memory` natively (Gemini, Antigravity, Cursor, Copilot, Codex), persistent agent context can be delivered through other xcaffold kinds:

| Pattern | Kind | How It Works |
|---------|------|--------------|
| Ambient instructions | [`context`](./context.md) | Body compiled into the root instruction file (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`). The agent or subagent reads it every session. |
...
| Session bootstrap | [`hooks`](./hooks.md) | A `SessionStart` hook script that reads memory files from disk and injects content into the session. |
