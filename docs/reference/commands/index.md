...
|---|---|---|
| `--config <path>` | `""` | Provide direct execution paths explicitly targeting a static `project.xcaf` resource (or directories representing multi-file dependencies) preventing local directory walk fallback routines. |
| `-g, --global` | `false` | Instructs the CLI execution layer resolving operations directly against user-wide structural constraints inside `~/.xcaffold/global.xcaf`. |
...
[**`apply`**](./lifecycle/apply.md) — Compile .xcaf resources into provider-native agent configuration files.
...
[**`validate`**](./utility/validate.md) — Check .xcaf syntax, cross-references, structural invariants, and policy compliance.
