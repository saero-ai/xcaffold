...
Antigravity always renders a native workflow primitive (`Kind: "workflow"`, `ID: "<name>"`). Unlike other providers, it does not go through the `rule-plus-skill` or basic lowering paths. The primitive's `Content` concatenates each step's instructions under a `## <step-name>` heading.
