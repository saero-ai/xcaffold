...
When you run `xcaffold translate --save-xcaf ir.xcaf`, the in-memory IR is serialized to disk as a `project.xcaf` file. This lets you inspect the IR before compilation, version-control it as a managed project, or feed it into `xcaffold apply` for ongoing GitOps management.
